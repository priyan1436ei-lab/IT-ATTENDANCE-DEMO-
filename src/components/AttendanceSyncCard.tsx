...
import { RefreshCw, Play, Square, ExternalLink, QrCode, PlusCircle, CheckCircle, Clock, AlertCircle, Copy, Check, Users } from 'lucide-react';
      ...
      {showQr && (
        <div className="bg-slate-50 border-b border-slate-200 p-6 flex flex-col items-center justify-center text-center animate-in fade-in">
          ...
          <div className="flex gap-2 mt-3">
            <button
              onClick={copyFormLink}
              className="text-xs text-purple-700 bg-purple-50 hover:bg-purple-100 border border-purple-200 font-medium px-3 py-1.5 rounded-lg transition"
            >
      ...
      <div className="p-4 bg-slate-50/70 border-b border-slate-200/70 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <button
            ...
            disabled={isSyncing}
            className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-semibold bg-emerald-600 hover:bg-emerald-700 text-white transition shadow-xs disabled:opacity-50 cursor-pointer"
          >
            <RefreshCw className={`w-3.5 h-3.5 ${isSyncing ? 'animate-spin' : ''}`} />
            <span>{isSyncing ? 'Syncing Now...' : 'Sync Form ➔ Sheet'}</span>
          </button>
          ...
          >
            {autoSyncActive ? (
              <>
                ...
                <span>Auto-Sync Active (Every 15s)</span>
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-white text-slate-700 border border-slate-200 hover:bg-slate-50 transition cursor-pointer ml-auto md:ml-0"
          >
        ...
        <div className="flex items-center gap-4 text-xs">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-slate-600 font-medium">OD / Excused:</span>
          ...
          <div className="pl-2 border-l border-slate-200 text-slate-700 font-semibold">
            Total in Sheet: <span className="text-emerald-700">{totalMarked}</span>
          </div>
        </div>
      </div>
    </div>
